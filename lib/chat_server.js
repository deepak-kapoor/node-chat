...
function handleNameChangeAttempts(socket, nickNames, namesUsed) {
	socket.on('nameAttempt', function(name) {
		if(name.indexOf('Guest') == 0) {
			socket.emit('nameResult', { succes: false, nessage: 'Name cannot begin with Guest' });
		} else {
			if(nameUsed.indexOf(name) == -1) {
				...
				socket.emit('nameResult', { success: false, message: 'That name is already taken' });
			}
		}
